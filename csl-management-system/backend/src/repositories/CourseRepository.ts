...
export class CourseRepository {
  ...
  async create(data: CreateCourseDTO): Promise<Course> {
    ...
    try {
      ...
      logger.info('Course created', { course_code: data.course_code });
      return result.rows[0];
    } catch (error) {
      logger.error('Failed to create course', { data, error });
      throw error;
    }
  }
  ...
  async findById(id: number): Promise<Course | null> {
    ...
    try {
      ...
      logger.error('Failed to find course by ID', { id, error });
      throw error;
    }
  }
  ...
  async findByCourseCode(courseCode: string): Promise<Course | null> {
    ...
    try {
      const result = await query(sql, [courseCode]);
      return result.rows[0] || null;
    } catch (error) {
      logger.error('Failed to find course by code', { courseCode, error });
      throw error;
    }
  }
  ...
  async update(id: number, data: UpdateCourseDTO): Promise<Course> {
    ...
    try {
      ...
      logger.error('Failed to update course', { id, data, error });
      throw error;
    }
  }
  ...
  async delete(id: number): Promise<void> {
    ...
    try {
      const result = await query(sql, [new Date(), id]);
      if (result.rowCount === 0) {
        throw new Error('Course not found');
      }
      logger.info('Course deleted (soft)', { id });
    } catch (error) {
      logger.error('Failed to delete course', { id, error });
      throw error;
    }
  }
  ...
  async search(
    ...
    try {
      ...
      logger.error('Failed to search courses', { filters, pagination, error });
      throw error;
    }
  }
  ...
  async findAllActive(): Promise<Course[]> {
    ...
    try {
      ...
      logger.error('Failed to find active courses', { error });
      throw error;
    }
  }
  ...
  async findByCategory(category: string): Promise<Course[]> {
    ...
    try {
      ...
      logger.error('Failed to find courses by category', { category, error });
      throw error;
    }
  }
   ...
   */
  async existsByCourseCode(courseCode: string, excludeId?: number): Promise<boolean> {
    let sql = 'SELECT 1 FROM courses WHERE course_code = $1';
    const values: any[] = [courseCode];
    ...
    try {
      ...
      logger.error('Failed to check course code existence', { courseCode, error });
      throw error;
    }
  }
