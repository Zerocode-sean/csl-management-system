...
router.get('/:id',
  [
    param('id').isInt().withMessage('Valid course ID is required')
  ],
  asyncHandler(async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
router.put('/:id',
  ...
  [
    param('id').isInt().withMessage('Valid course ID is required'),
    body('title').optional().isLength({ max: 255 }).withMessage('Course title must be less than 255 characters'),
    body('description').optional().isString(),
...
router.delete('/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (certificateCount > 0) {
      throw createError(
        `Cannot delete course with ${certificateCount} associated certificate(s). Please archive the course instead.`,
        400
      );
    }
...
router.patch('/:id/deactivate',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (result.rows.length === 0) {
      throw createError('Course not found or already deactivated', 404);
    }
