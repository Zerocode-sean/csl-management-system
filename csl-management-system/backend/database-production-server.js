...
const DatabaseRepository = {
  async getStudents(limit = 10, offset = 0, search = '') {
    ...
    const query = `
      SELECT student_id, name, email, mobile, address, date_of_birth, 
             profile_picture, institution, grade, created_at, updated_at, deleted_at
      FROM students 
      ${searchQuery}
  ...
  async createStudent(studentData) {
    // Support optional profile picture (base64 or URL) and map frontend fields
    const { student_id, studentId, student_custom_id, name, email, mobile, date_of_birth, address, profilePicture, profile_picture, institution, grade } = studentData;
    const profilePic = profilePicture || profile_picture || null;
    const sid = student_id || student_custom_id || studentId || null;
    const result = await pool.query(
      `INSERT INTO students (student_id, name, email, mobile, date_of_birth, address, profile_picture, institution, grade, enrollment_date)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_DATE)
       RETURNING *`,
      [sid, name, email, mobile, date_of_birth, address, profilePic, institution || null, grade || null]
    );
    return result.rows[0];
  },

  async updateStudent(id, studentData) {
    const { name, email, mobile, date_of_birth, address, profilePicture, profile_picture, institution, grade } = studentData;
    const profilePic = profilePicture || profile_picture || null;
    const result = await pool.query(
      `UPDATE students 
       SET name = $1, email = $2, mobile = $3, 
           date_of_birth = $4, address = $5, profile_picture = $6, institution = $7, grade = $8, updated_at = NOW()
       WHERE student_id = $9
       RETURNING *`,
      [name, email, mobile, date_of_birth, address, profilePic, institution || null, grade || null, id]
    );
    return result.rows[0];
  },
...
app.post(`${API_PREFIX}/students`, async (req, res) => {
  try {
    ...
    // Basic validation - check for required fields from frontend
    ...
    if (isDatabaseConnected) {
      ...
      const dbData = {
        ...
        // accept either snake_case or camelCase date field from frontend
        date_of_birth: studentData.date_of_birth || studentData.dateOfBirth || null,
        address: studentData.address,
        profilePicture: studentData.profilePicture || studentData.profile_picture || null,
        institution: studentData.institution || null,
        grade: studentData.grade || null
      };
      newStudent = await DatabaseRepository.createStudent(dbData);
    } else {
      newStudent = {
        student_id: mockData.students.length + 1,
        ...studentData,
...
app.put(`${API_PREFIX}/students/:id`, async (req, res) => {
  try {
    ...
    if (isDatabaseConnected) {
      ...
      updatedStudent = await DatabaseRepository.updateStudent(id, dbData);
    } else {
...
app.post(`${API_PREFIX}/courses`, async (req, res) => {
  try {
    ...
    if (!courseData.course_code || !courseData.course_name) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: course_code, course_name'
      });
    }
    
    let newCourse;
    if (isDatabaseConnected) {
      newCourse = await DatabaseRepository.createCourse(courseData);
    } else {
      newCourse = {
