...
export class CertificateService {
    ...
    try {
      ...
      for (const logoPath of possibleLogoPaths) {
        try {
          ...
          logger.warn('Failed to load logo from:', logoPath);
        }
      }

      if (!logoBase64) {
        logger.warn('Logo file not found in any location, using placeholder');
  ...
  static async issueCertificate(certificateData: CertificateData): Promise<any> {
    try {
      ...
      logger.info('Certificate dates calculated:', {
