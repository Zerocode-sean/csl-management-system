...
router.get('/logs',
  ...
  [
    ...
    queryValidator('action').optional().isIn(['CREATE', 'UPDATE', 'DELETE']),
    queryValidator('table_name').optional().isString().trim(),
    queryValidator('admin_id').optional().isUUID(),
    queryValidator('start_date').optional().isDate(),
    queryValidator('end_date').optional().isDate()
  ],
...
router.get('/export',
  ...
  [
    queryValidator('start_date').isDate().withMessage('Start date is required'),
    queryValidator('end_date').isDate().withMessage('End date is required'),
    queryValidator('format').optional().isIn(['json', 'csv']).withMessage('Format must be json or csv')
  ],
  asyncHandler(async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    if (format === 'csv') {
      ...
      logsResult.rows.forEach((row: any) => {
        const csvRow = [
          row.log_id,
          row.admin_id || '',
          `"${row.admin_name || ''}"`,
