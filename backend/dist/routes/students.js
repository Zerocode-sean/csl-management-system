...
router.get('/', [
    ...
    let whereConditions = ['s.deleted_at IS NULL']; // Only show non-deleted students
    let queryParams = [];
    let paramIndex = 1;
    // Active filter
    ...
    // Sanitize large profile pictures in list view to prevent JSON corruption
    const sanitizedStudents = studentsResult.rows.map((student) => {
        if (student.profile_picture && student.profile_picture.length > 100) {
            return {
                ...student,
                profile_picture: `<base64 image ${(student.profile_picture.length / 1024).toFixed(2)}KB>`,
...
router.get('/:id', [
    ...
    const studentResult = await (0, connection_1.query)(`
      SELECT 
        ...
        COUNT(c.csl_number) FILTER (WHERE c.status = 'active') as active_certificates
      FROM students s
...
router.get('/:id/profile-picture', [
    ...
    res.json({
        ...
        data: {
            ...
            size: profilePicture ? profilePicture.length : 0
        }
    });
}));
/**
...
router.post('/', (0, auth_1.authorizeRoles)('super_admin', 'admin', 'course_manager'), [
    ...
    const { name, email, address, date_of_birth, student_custom_id, course_id } = req.body;
    // Handle field aliases
    const phone = req.body.phone || req.body.mobile; // Support both 'phone' and 'mobile'
    ...
    try {
      ...
      `, [
        ...
        // Remove large profile_picture from response to prevent JSON corruption
        const { profile_picture: _, ...studentWithoutPicture } = student;
        res.status(201).json({
            success: true,
            message: 'Student created successfully',
            data: {
                ...studentWithoutPicture,
                profile_picture: profile_picture ? `<base64 image ${(profile_picture.length / 1024).toFixed(2)}KB>` : null
            }
        });
    }
    catch (error) {
...
router.put('/:id', (0, auth_1.authorizeRoles)('super_admin', 'admin', 'course_manager'), [
    ...
    if (course_id !== undefined && course_id !== null && course_id !== '') {
        ...
        const enrollmentCheck = await (0, connection_1.query)('SELECT student_course_id FROM student_courses WHERE student_id = $1 AND course_id = $2', [id, course_id]);
        ...
        `, [id, course_id]);
            logger_1.logger.info('Student enrolled in course:', {
                studentId: id,
                courseId: course_id
            });
        }
    ...
    // Remove large profile_picture from response to prevent JSON corruption
    const { profile_picture: profilePic, ...studentWithoutPicture } = updatedStudent;
    res.json({
        success: true,
        message: 'Student updated successfully',
        data: {
            ...studentWithoutPicture,
            profile_picture: profilePic ? `<base64 image ${(profilePic.length / 1024).toFixed(2)}KB>` : null
        }
    });
}));
/**
...
router.delete('/:id', (0, auth_1.authorizeRoles)('super_admin', 'admin'), [
    ...
    const currentResult = await (0, connection_1.query)('SELECT * FROM students WHERE student_id = $1 AND deleted_at IS NULL', [id]);
    if (currentResult.rows.length === 0) {
    ...
    // Don't return full student data with profile_picture (can be huge)
    const { profile_picture, ...studentWithoutPicture } = result.rows[0];
    res.json({
        success: true,
        message: 'Student deleted successfully',
        data: {
            ...studentWithoutPicture,
            profile_picture: profile_picture ? `<removed ${(profile_picture.length / 1024).toFixed(2)}KB>` : null
        }
    });
}));
