...
class CertificateRepository {
     ...
     */
    async issue(data, issuedBy) {
        const cslNumber = await (0, certificateUtils_1.generateCSLNumber)();
        const certificateNumber = `CERT-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
        const verificationHash = (0, certificateUtils_1.generateVerificationHash)(cslNumber, data.student_id, data.course_id);
        const sql = `
    ...
    `;
        const values = [
            ...
            data.grade || null,
            data.gpa || null,
            models_1.CertificateStatus.ACTIVE,
            issuedBy,
            verificationHash
        ];
        try {
            const result = await (0, connection_1.query)(sql, values);
            logger_1.logger.info('Certificate issued', {
            ...
            logger_1.logger.error('Failed to issue certificate', { data, error });
            throw error;
        }
    }
    /**
    ...
    async findById(id) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to find certificate by ID', { id, error });
            throw error;
        }
    }
    /**
     ...
     */
    async findByCSLNumber(cslNumber) {
        const sql = 'SELECT * FROM certificates WHERE csl_number = $1';
        try {
            const result = await (0, connection_1.query)(sql, [cslNumber]);
            return result.rows[0] || null;
        }
        catch (error) {
            logger_1.logger.error('Failed to find certificate by CSL number', { cslNumber, error });
            throw error;
        }
    }
    /**
    ...
    `;
        try {
            ...
            if (cert.status !== models_1.CertificateStatus.ACTIVE) {
                return {
                    valid: false,
            ...
            logger_1.logger.error('Failed to verify certificate', { cslNumber, error });
            return {
                valid: false,
    ...
    `;
        try {
            const result = await (0, connection_1.query)(sql, [status, new Date(), id]);
            if (result.rows.length === 0) {
                throw new Error('Certificate not found');
            }
            logger_1.logger.info('Certificate status updated', { id, status, updated_by: updatedBy });
            return result.rows[0];
        }
        catch (error) {
            logger_1.logger.error('Failed to update certificate status', { id, status, error });
            throw error;
        }
    }
    /**
    ...
    `;
        ...
        try {
            ...
            logger_1.logger.error('Failed to search certificates', { filters, pagination, error });
            throw error;
        }
    }
    /**
    ...
    `;
        try {
            const result = await (0, connection_1.query)(sql, [studentId]);
            return result.rows;
        }
        catch (error) {
            logger_1.logger.error('Failed to find certificates by student', { studentId, error });
            throw error;
        }
    }
    /**
    ...
    `;
        try {
            const result = await (0, connection_1.query)(sql, [courseId]);
            return result.rows;
        }
        catch (error) {
            logger_1.logger.error('Failed to find certificates by course', { courseId, error });
            throw error;
        }
    }
    /**
     ...
     */
    async existsForStudentCourse(studentId, courseId) {
        const sql = `
    ...
    `;
        try {
            const result = await (0, connection_1.query)(sql, [studentId, courseId, models_1.CertificateStatus.ACTIVE]);
            return result.rows.length > 0;
        }
        catch (error) {
            logger_1.logger.error('Failed to check certificate existence', { studentId, courseId, error });
            throw error;
        }
    }
    /**
    ...
    `;
        try {
            const [totalResult, statusResult, recentResult] = await Promise.all([
                ...
                (0, connection_1.query)(statusSql),
                (0, connection_1.query)(recentSql)
            ]);
            return {
            ...
            logger_1.logger.error('Failed to get certificate statistics', { error });
            throw error;
        }
    }
}
