...
exports.CSLGeneratorService = void 0;
...
class CSLGeneratorService {
     ...
     */
    static async generateCSLNumber(studentId, courseId) {
        try {
            // 1. Get course code
            const pool = (0, connection_1.getPool)();
            const courseResult = await pool.query('SELECT code FROM courses WHERE course_id = $1', [courseId]);
            ...
            const sequenceResult = await pool.query('SELECT get_next_sequential_number($1, $2) as next_number', [year, courseCode]);
            ...
            const fullHash = crypto_1.default
                .createHash('sha256')
                .update(hashInput)
                .digest('hex');
            ...
            logger_1.logger.info('CSL number generated', {
                cslNumber,
                studentId,
                courseId,
                year,
            ...
            logger_1.logger.error('Failed to generate CSL number', { error, studentId, courseId });
            throw error;
        }
    }
    /**
     ...
     */
    static async verifyCSLNumber(cslNumber) {
        try {
            // 1. Validate format
            const parts = cslNumber.split('-');
            if (parts.length !== 4) {
                logger_1.logger.warn('Invalid CSL format: incorrect number of parts', { cslNumber });
                return false;
            }
            const [year, courseCode, sequence, providedHash] = parts;
            // Validate all parts exist
            if (!year || !courseCode || !sequence || !providedHash) {
                logger_1.logger.warn('Invalid CSL format: missing parts', { cslNumber });
                return false;
            }
            // Validate year format (4 digits)
            if (!/^\d{4}$/.test(year)) {
                logger_1.logger.warn('Invalid CSL format: invalid year', { cslNumber, year });
                return false;
            }
            // Validate course code format (2 uppercase letters)
            if (!/^[A-Z]{2}$/.test(courseCode)) {
                logger_1.logger.warn('Invalid CSL format: invalid course code', { cslNumber, courseCode });
                return false;
            }
            // Validate sequence format (4 digits)
            if (!/^\d{4}$/.test(sequence)) {
                logger_1.logger.warn('Invalid CSL format: invalid sequence', { cslNumber, sequence });
                return false;
            }
            // Validate hash format (6 alphanumeric uppercase)
            if (!/^[A-Z0-9]{6}$/.test(providedHash)) {
                logger_1.logger.warn('Invalid CSL format: invalid hash', { cslNumber, providedHash });
                return false;
            }
            // 2. Get certificate from database
            const pool = (0, connection_1.getPool)();
            const certResult = await pool.query('SELECT student_id FROM certificates WHERE csl_number = $1', [cslNumber]);
            if (certResult.rows.length === 0) {
                logger_1.logger.warn('CSL number not found in database', { cslNumber });
                return false;
            }
            ...
            if (!isValid) {
                logger_1.logger.warn('CSL cryptographic verification failed', {
                    cslNumber,
                    expectedHash,
                    providedHash,
                    studentId
                ...
                logger_1.logger.info('CSL verified successfully', { cslNumber });
            }
            return isValid;
        }
        catch (error) {
            logger_1.logger.error('CSL verification error', { error, cslNumber });
            return false;
        }
    }
    /**
...
exports.CSLGeneratorService = CSLGeneratorService;
