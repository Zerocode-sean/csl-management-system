...
const certificateGenerator_service_1 = require("../services/certificateGenerator.service");
...
const runTest = async () => {
    try {
        ...
        let adminId;
        const adminResult = await (0, connection_1.query)("SELECT admin_id FROM admins LIMIT 1");
        if (adminResult.rows.length > 0) {
            ...
            const newAdmin = await (0, connection_1.query)(`
      ...
      `);
        ...
        let studentId;
        const studentResult = await (0, connection_1.query)("SELECT student_id FROM students LIMIT 1");
        if (studentResult.rows.length > 0) {
            studentId = studentResult.rows[0].student_id;
            console.log(`Using existing student ID: ${studentId}`);
        }
        else {
            const newStudent = await (0, connection_1.query)(`
      ...
      `);
        ...
        let courseId;
        const courseResult = await (0, connection_1.query)("SELECT course_id FROM courses LIMIT 1");
        if (courseResult.rows.length > 0) {
            courseId = courseResult.rows[0].course_id;
            console.log(`Using existing course ID: ${courseId}`);
        }
        else {
            const newCourse = await (0, connection_1.query)(`
      ...
      `);
        ...
        await (0, connection_1.query)("DELETE FROM certificates WHERE student_id = $1 AND course_id = $2", [studentId, courseId]);
        console.log('Cleaned up any existing certificates for this student/course');
        // 5. Generate Certificate
        console.log('Generating certificate...');
        const result = await certificateGenerator_service_1.CertificateGeneratorService.generateCertificate(studentId, courseId, adminId);
        console.log('Certificate generated successfully!');
        console.log('CSL Number:', result.cslNumber);
        console.log('PDF Path:', result.pdfPath);
        // 6. Verify in database
        const certCheck = await (0, connection_1.query)("SELECT * FROM certificates WHERE csl_number = $1", [result.cslNumber]);
