...
const StudentsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        toast.error('Failed to Load Courses: Could not load courses list. Please refresh the page.');
      } finally {
  ...
  // Function to load profile pictures separately
  const loadProfilePictures = async (studentsToLoad: Student[]) => {
    const picturesMap: Record<string, string> = {};
    
    for (const student of studentsToLoad) {
      if (student.id && !student.profilePicture) {
        try {
          const response = await studentsService.getProfilePicture(Number(student.id));
          if (response.success && response.data.profile_picture) {
            picturesMap[student.id] = response.data.profile_picture;
          }
        } catch (error) {
          console.error(`Failed to load profile picture for student ${student.id}:`, error);
        }
      } else if (student.profilePicture && !student.profilePicture.startsWith('<base64 image')) {
        // Already has a valid picture
        picturesMap[student.id] = student.profilePicture;
      }
    }
    ...
    const fetchStudents = async () => {
      ...
      try {
        ...
        const mappedStudents = studentsRaw.map((student: any) => {
          // Check if profile_picture is a placeholder text from backend
          const profilePic = student.profile_picture ?? student.profilePicture ?? '';
          const isPlaceholder = profilePic.startsWith('<base64 image');
          ...
          return {
            ...
            profilePicture: isPlaceholder ? '' : profilePic, // Don't use placeholder as URL
        ...
        // Load profile pictures separately for students that need them
        loadProfilePictures(mappedStudents);
      } catch (error) {
        ...
        toast.error('Using Demo Data: Could not connect to backend. Showing sample data.');
        setStudents([
          {
            id: '1',
            name: 'John Doe',
  ...
  const handleSaveStudent = async () => {
    ...
    try {
      ...
      const pictureInfo = newStudent.profilePicture 
        ? ` (with profile picture: ${formatFileSize(getBase64Size(newStudent.profilePicture))})`
        : '';
      toast.success(`Student Added Successfully! ${newStudent.name} has been added and saved to the database${pictureInfo}.`);

      // Refresh students list from backend
      try {
        console.log('🔄 Fetching updated students list...');
        // Add cache-busting parameter to ensure fresh data
        const refreshed = await studentsService.getAllStudents({ _ts: Date.now() } as any);
        console.log('Refresh response:', refreshed);
        
        // Check if response has students nested in data.students or directly in data
        const studentsArray = refreshed.data?.students || refreshed.data;
        
        if (refreshed.success && Array.isArray(studentsArray)) {
          const mappedStudents = studentsArray.map((student: any) => ({
            id: student.student_id?.toString() || student.id?.toString() || '',
            name: student.name || student.full_name || '',
            email: student.email || '',
            phone: student.phone || student.phone_number || student.mobile || '',
            studentId: student.student_custom_id || student.studentId || '',
            address: student.address || '',
            dateOfBirth: student.date_of_birth || student.dateOfBirth || '',
            ...
            certificates: student.certificates_count || student.certificate_count || student.certificates || 0,
            status: student.status || 'active',
            registrationDate: student.registration_date || student.registrationDate || student.created_at || new Date().toISOString().split('T')[0],
            lastActive: student.last_active || student.lastActive || 'Recently'
          }));
          setStudents(mappedStudents);
          console.log(`✅ Students list refreshed successfully: ${mappedStudents.length} students loaded`);
          console.log('New student should be:', newStudent.name);
          
          // Verify the new student is in the list
          const addedStudent = mappedStudents.find((s: any) => 
            s.name === newStudent.name || s.email === newStudent.email
          );
          if (addedStudent) {
            console.log('✅ New student found in refreshed list:', addedStudent);
          } else {
            console.warn('⚠️ New student NOT found in refreshed list. This might be a backend issue.');
          }
        } else {
          console.warn('Failed to refresh students list:', refreshed);
          toast.error('Student added but list update failed. Please refresh the page.');
        }
      } catch (refreshError) {
        console.error('Error refreshing students:', refreshError);
        // Don't fail the whole operation if refresh fails
        toast.error('Student added but failed to refresh list. Please reload the page.');
      }
      ...
      toast.error('Failed to Add Student. An error occurred while adding the student. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const confirmDeleteStudent = async () => {
    ...
    try {
      ...
      const result = await studentsService.deleteStudent(parseInt(studentToDelete.id) || 0);
      ...
      if (result && (result.success || result.success === undefined)) {
        ...
        // Show success toast
        toast.success(`Student Deleted: ${studentToDelete.name} has been removed from the system.`);
  ...
  const handleSaveEditStudent = async () => {
    ...
    try {
      ...
      toast.success(`Student Updated Successfully: ${editStudent.name} has been updated and saved to the database.`);
      ...
      toast.error('Failed to Update Student. An error occurred while updating the student. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleGenerateCertificate = async (student: Student) => {
    ...
    if (student.enrolledCourses.length === 0) {
      toast.error('No Courses Enrolled: Student has no enrolled courses to generate certificate for');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success('Certificate Generated: Certificate generated and downloaded successfully!');
      } else {
      ...
      toast.error(`Certificate Generation Failed: ${error instanceof Error ? error.message : 'Failed to generate certificate'}`);
    }
  };
                    ...
                    <div className="flex items-center">
                      <div className="h-11 w-11 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center shadow-lg overflow-hidden">
                        {profilePictures[student.id] ? (
                          <img 
                            src={profilePictures[student.id]} 
                            alt={student.name}
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          if (validationError) {
                            setFormErrors(prev => ({ ...prev, profilePicture: validationError }));
                          ...
                          try {
                            ...
                            const compressedSize = formatFileSize(getBase64Size(compressed));
                            console.log(`Compressed to: ${compressedSize}`);

                            setNewStudent({ ...newStudent, profilePicture: compressed });
                            toast.success(`Image Uploaded: Compressed from ${originalSize} to ${compressedSize}`);
                          } catch (error) {
                            console.error('Image compression error:', error);
                            setFormErrors(prev => ({ 
                              ...prev, 
                              profilePicture: 'Failed to process image. Please try another file.' 
                            }));
                          } finally {
                  ...
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    {newStudent.profilePicture ? (
                      <span className="text-green-600 dark:text-green-400">
                        ✓ Image compressed to {formatFileSize(getBase64Size(newStudent.profilePicture))}
                      </span>
                    ) : (
                      'Optional: Upload a profile photo (JPG, PNG, max 5MB). Images will be automatically compressed.'
                    )}
                  </p>
                </div>
                      ...
                      onClick={() => {
                        if (editStudent.courseId) {
                          ...
                          if (selectedCourse) {
                            ...
                            toast.success(`Course Added: ${selectedCourse.title} has been added`);
                          }
                        }
                      }}
