...
// In production, this becomes a relative path to the deployed API
const api = axios.create({
  baseURL: '/api/v1',
  timeout: 30000, // Increased to 30 seconds for certificate generation and large operations
  headers: {
...
api.interceptors.request.use(
  (config) => {
    // const { authService } = require('../services/authService'); // Removed require
...
export const certificatesApi = {
  ...
  downloadPdf: (cslNumber: string) =>
    api.get(`/certificates/${cslNumber}/download`, { 
      responseType: 'blob' 
    }),
}

// Verification API (Public - No Auth Required)
// Create separate axios instance for public endpoints (no auth token)
...
export const verificationApi = {
  verify: (cslNumber: string) =>
    publicApi.get(`/public/verify/${cslNumber}`),
}
